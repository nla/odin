...
/**
 * Used to listen for and respond to method calls via implementations of
 * {@link com.google.gwt.user.client.rpc.RemoteService} from web clients made
 * over HTTP. This class creates a service that listens for GWT RPC requests
 * over HTTP. This adapter can be used instead of embedding the application into
 * an application server framework. If using a third-party application server
 * framework, this adapter need not be added to the adapter.properties file.
 */
